...
* 🚀 **Instant Dashboard Startup:** the pair list is read as ONE `UNION ALL` query per 120 tables (not one query per pair) and is **type-stable** — a legacy table whose `ob_*` columns are TEXT no longer kills its whole chunk, so the scan never degrades into thousands of single-table reads. Mixed columns are flattened to TEXT in SQL and converted back in Python, a chunk that still fails retries once as an all-TEXT query, and the scan is bounded by `DASH_SCAN_BUDGET_SEC` (renders what it has, never caches a truncated list as the startup snapshot).
