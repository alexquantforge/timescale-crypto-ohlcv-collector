...
* 🖱 **Instant Pair Switching:** the Charts tab paints the candles already stored in TimescaleDB first — gap stitching and the live ticker / orderbook / trade-tape feeds run in background daemon threads and are swapped in when they land. A swap repaints only when it actually filled candles, so browsing healthy pairs never resets your zoom.
