...
When a market list hangs, the load is the thing to measure, not the API: `DASH_MARKET_LOAD_DEBUG=true`
prints `[markets] gate: probe, 3 categories, 90s per request — spot FAILED after 91.8s …; future ok,
30 markets, 2.8s`, and re-asks any hanging leg **once** with `Accept-Encoding: identity`, because that
is the only thing a `curl` that finishes in 3s does differently. If every hanging leg then answers, the
load is retried that way in the same cycle (feeds and stitching stop being blind for the length of the
backoff) and the conclusion is kept for the process; the answer they should paste is that line.
`DASH_MARKET_LOAD_ACCEPT_ENCODING=identity` states it as a setting instead — one header on the load
path, zero extra requests, and the 1-second feeds keep gzip. Nothing here is a retry policy: a failure
still costs one request per backoff window unless the knob is on, and the knob's probe runs once per
process.
